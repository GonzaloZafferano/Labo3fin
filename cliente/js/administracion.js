...
import {eliminarElementoPorIdAJAX, obtenerElementoPorIdAJAX, obtenerElementosAJAX, insertarElementoAJAX, actualizarElementoAJAX} from "./ajax.js";

import {eliminarElementoPorIdFETCH,obtenerElementoPorIdFETCH,obtenerElementosFETCH, insertarElementoFETCH, actualizarElementoFETCH, eliminarElementoPorIdFETCHconPromesa, insertarElementoFETCHconPromesa} from "./fetch.js";

import {eliminarElementoPorIdAXIOS,obtenerElementoPorIdAXIOS,obtenerElementosAXIOS,insertarElementoAXIOS,actualizarElementoAXIOS, eliminarElementoPorIdAXIOSconPromesa, insertarElementoAXIOSconPromesa} from "./axios.js";
        ...
        if(!existenInputsInvalidos(e.target.elements) && inyeccion.value && (gato.checked || perro.checked)){
            ...
            if (!idItem) {
                if(sePuedeOperar){
                    ...
                    sePuedeOperar = false;
                    setSpinner();
                    //insertarElementoFETCH(URL, item)   
                    //insertarElementoAXIOS(URL, item)   
                    insertarElementoAJAX(URL, item)   
                    //insertarElementoAXIOSconPromesa(URL, item)  
                    //insertarElementoFETCHconPromesa(URL, item)  
                ...
                if(confirm("Realmente desea modificar la mascota?")){   
                    if(sePuedeOperar){
                        ...
                        sePuedeOperar = false;
                        setSpinner();
                       //actualizarElementoFETCH(URL, item)
...
eliminar.addEventListener("click", (e) =>{
    ...
    if(sePuedeOperar){
        ...
        sePuedeOperar = false;
        setSpinner();
    //eliminarElementoPorIdFETCH(URL, idItem)
    //eliminarElementoPorIdAXIOS(URL, idItem)
    eliminarElementoPorIdAJAX(URL, idItem)
    //eliminarElementoPorIdAXIOSconPromesa(URL, idItem)
    //eliminarElementoPorIdFETCHconPromesa(URL, idItem)
        .then(x => actualizarTabla())
        .catch(error => console.error(error))
        .finally(X =>{
        sePuedeOperar = true;
...
$tabla.addEventListener("click", (e) => {
    ...
    if (elemento.matches("td")) {
       ...
       if(sePuedeOperar){
        sePuedeOperar = false;
        setSpinner();
        //obtenerElementoPorIdFETCH(URL, itemId)
        //obtenerElementoPorIdAXIOS(URL, itemId)
        obtenerElementoPorIdAJAX(URL, itemId)
...
async function actualizarTabla() {
    ...
    setSpinner();
   //await obtenerElementosFETCH(URL)
   //await obtenerElementosAXIOS(URL)
